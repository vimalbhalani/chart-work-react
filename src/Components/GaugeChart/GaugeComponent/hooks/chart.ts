...
export const initChart = (gauge: Gauge, isFirstRender: boolean) => {
    ...
    const updatedValue = (JSON.stringify(gauge.prevProps.current.value) !== JSON.stringify(gauge.props.value));
...
export const renderChart = (gauge: Gauge, resize: boolean = false) => {
    ...
    if (resize) {
        ...
        const gaugeTypeHeightCorrection: Record<string, number> = {
            [GaugeType.Semicircle]: 50,
            [GaugeType.Radial]: 55,
            [GaugeType.Grafana]: 55
        }
        const boundHeight = gauge.doughnut.current.node().getBoundingClientRect().height;
        const boundWidth = gauge.container.current.node().getBoundingClientRect().width;
        const gaugeType = gauge.props.type as string;
        gauge.svg.current
            .attr("width", boundWidth)
            .attr("height", boundHeight + gaugeTypeHeightCorrection[gaugeType]);
    } else {
        const arcsPropsChanged = (JSON.stringify(gauge.prevProps.current.arc) !== JSON.stringify(gauge.props.arc));
        const pointerPropsChanged = (JSON.stringify(gauge.prevProps.current.pointer) !== JSON.stringify(gauge.props.pointer));
        const valueChanged = (JSON.stringify(gauge.prevProps.current.value) !== JSON.stringify(gauge.props.value));
        const ticksChanged = (JSON.stringify(gauge.prevProps.current.labels?.tickLabels) !== JSON.stringify(labels.tickLabels));
        const shouldRedrawArcs = arcsPropsChanged
        if (shouldRedrawArcs) {
        ...
        const shouldRedrawPointer = pointerPropsChanged || (valueChanged && !gauge.props?.pointer?.hide);
...
export const updateDimensions = (gauge: Gauge) => {
    ...
    const { dimensions } = gauge;
    const divDimensions = gauge.container.current.node().getBoundingClientRect(),
    ...
    const isMarginBox = typeof marginInPercent == 'number';
    const marginLeft: number = isMarginBox ? marginInPercent as number : (marginInPercent as GaugeInnerMarginInPercent).left;
    const marginRight: number = isMarginBox ? marginInPercent as number : (marginInPercent as GaugeInnerMarginInPercent).right;
    const marginTop: number = isMarginBox ? marginInPercent as number : (marginInPercent as GaugeInnerMarginInPercent).top;
    const marginBottom: number = isMarginBox ? marginInPercent as number : (marginInPercent as GaugeInnerMarginInPercent).bottom;

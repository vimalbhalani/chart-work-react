...
export const setupValueLabel = (gauge: Gauge) => {
  ...
  if (!labels?.unitLabel?.hide) addUnitText(gauge)
}
...
export const addTickLine = (tick: Tick, gauge: Gauge) => {
  const { labels } = gauge.props;
  const { angle } = calculateAnchorAndAngleByValue(tick?.value as number, gauge);
  let tickDistanceFromArc = tick.lineConfig?.distanceFromArc || labels?.tickLabels?.defaultTickLineConfig?.distanceFromArc || 0;
  ...
  const tickColor = tick.lineConfig?.color || labels?.tickLabels?.defaultTickLineConfig?.color || defaultTickLabels.defaultTickLineConfig?.color;
  const tickWidth = tick.lineConfig?.width || labels?.tickLabels?.defaultTickLineConfig?.width || defaultTickLabels.defaultTickLineConfig?.width;
  const tickLength = tick.lineConfig?.length || labels?.tickLabels?.defaultTickLineConfig?.length || defaultTickLabels.defaultTickLineConfig?.length as number;
...
export const addTickValue = (tick: Tick, gauge: Gauge) => {
  ...
  const tickDistanceFromArc = tick.lineConfig?.distanceFromArc || labels?.tickLabels?.defaultTickLineConfig?.distanceFromArc || 0;
  ...
  let text = '';
  const maxDecimalDigits = tick.valueConfig?.maxDecimalDigits || labels?.tickLabels?.defaultTickValueConfig?.maxDecimalDigits;
...
export const addValueText = (gauge: Gauge) => {
  ...
  const valueLabel = labels?.valueLabel as ValueLabel;
  const textPadding = 20;
  let text = valueLabel?.text || '';
  // const maxDecimalDigits = labels?.valueLabel?.maxDecimalDigits;
  ...
  const valueFontSize = valueLabel?.style?.fontSize as string;
  const valueTextStyle = { ...valueLabel.style };
  const x = gauge.dimensions.current.outerRadius;
  let y = 0;
  ...
  //}
  const widthFactor = gauge.props.type == GaugeType.Radial ? 0.003 : 0.003;
  fontRatio = gauge.dimensions.current.width * widthFactor * fontRatio;
  const fontSizeNumber = parseInt(valueFontSize, 10) * fontRatio;
...
//Adds text near the value text
export const addUnitText = (gauge: Gauge) => {
  const { labels } = gauge.props;
  const unitLabel = labels?.unitLabel as ValueLabel;
  const textPadding = 20;
  let text = unitLabel?.text || '';
  const maxLengthBeforeComputation = 4;
  const textLength = text?.length || 0;
  let fontRatio = textLength > maxLengthBeforeComputation ? maxLengthBeforeComputation / textLength * 1.5 : 1; // Compute the font size ratio
  const valueFontSize = unitLabel?.style?.fontSize as string;
  const unitTextStyle = { ...unitLabel?.style };
  const x = gauge.dimensions.current.outerRadius + 20;
  let y = 0;
  unitTextStyle.textAnchor = "start";
  if (gauge.props.type == GaugeType.Semicircle) {
    y = gauge.dimensions.current.outerRadius / 1.5 + textPadding;
  } else if (gauge.props.type == GaugeType.Radial) {
    y = gauge.dimensions.current.outerRadius * 1.45 + textPadding;
  ...
  fontRatio = gauge.dimensions.current.width * widthFactor * fontRatio;
  const fontSizeNumber = parseInt(valueFontSize, 10) * fontRatio;
  unitTextStyle.fontSize = fontSizeNumber + "px";
  addText(text, x, y, gauge, unitTextStyle, CONSTANTS.valueLabelClassname);
};
...
export const calculateAnchorAndAngleByValue = (value: number, gauge: Gauge) => {
  ...
  const isValueBetweenTolerance = valuePercentage > CONSTANTS.rangeBetweenCenteredTickValueLabel[0] &&

        ...
        gauge.doughnut.current
            ...
            .tween("progress", () => {
                ...
                return function (percentOfPercent: number) {
                    ...
                    if (isProgressValid(progress, prevProgress)) {
...
const setupContext = (gauge: Gauge): PointerContext => {
    ...
    const length = pointer.type == PointerType.Needle ? pointerLength : 0.2;
...
const initPointer = (gauge: Gauge) => {
    ...
    const pointer = gauge.props.pointer as PointerProps;
    const { shouldDrawPath, centerPoint, pointerRadius, currentPercent, prevPercent } = gauge.pointer.current.context;
    if(shouldDrawPath){
...
const isProgressValid = (currentPercent: number, prevPercent: number) => {

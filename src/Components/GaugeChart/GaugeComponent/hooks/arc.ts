...
const onArcMouseOut = (event: any, _d: any, _gauge: Gauge) => {
...
export const setArcData = (gauge: Gauge) => {
  ...
  if (arc?.subArcs && !arc?.nbSubArcs) {
    ...
    const subArcsLength: Array<number> = [];
    const subArcsLimits: Array<number> = [];
    const subArcsTooltip: Array<Tooltip> = [];
...
const getGrafanaMainArcData = (gauge: Gauge, percent: number | undefined = undefined) => {
  const currentPercentage = percent != undefined ? percent : utils.calculatePercentage(gauge.props.minValue as number,
...
const drawGrafanaOuterArc = (gauge: Gauge, resize: boolean = false) => {
  ...
  if (gauge.props.type == GaugeType.Grafana && resize) {
    ...
    const outerArcSubarcs = arcPaths
      .append("path")
      .attr("d", outerArc);
...
export const applyColors = (subArcsPath: any, gauge: Gauge) => {
  if (gauge.props?.arc?.gradient) {
    ...
    subArcsPath.style("fill", (_d: any) => `url(#${uniqueId})`);
  } else {
...
export const getColors = (nbArcsToDisplay: number, gauge: Gauge) => {
  ...
  if (!arc.colorArray) {
    const subArcColors = arc.subArcs?.map((subArc) => subArc.color);
...
export const createGradientElement = (div: any, uniqueId: string) => {
  ...
  const lg = div.append("defs").append("linearGradient")
